...
// Variáveis de estado da roleta (agora isoladas neste módulo)
let startAngle = 0;
...
// ========================== DESENHO DA ROLETA (adaptável à tela) ==========================
window.drawRoulette = function() {
    ...
    const radius = Math.min(width, height) * 0.46; // 46% do menor lado
    ...
    const borderWidth = radius * 0.045; // borda externa

    // Borda externa com sombra
    ctx.beginPath();
    ...
    for (let i = 0; i < numSegments; i++) {
        ...
        ctx.shadowBlur = 8;
        ctx.fillText(items[i], radius * 0.78, 0);
        ctx.shadowBlur = 0;
        ctx.restore();
    }
...
function finalizeSpin() {
    ...
    // Cálculo correto do índice (marcador no topo = -PI/2)
    ...
    // Garantir que o índice esteja dentro do range
    if (index < 0) index = 0;
    ...
    setTimeout(() => {
        ...
        window.launchConfetti(); // (efeitos.js)
